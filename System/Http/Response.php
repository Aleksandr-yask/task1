...
class Response {
    ...
    public function sendError($code) {
        $this->setContent(['status' => 'error', 'code' => $code, 'body' => $this->statusTexts[$code]]);
    }
     ...
     * @param int $code
     */
    public function setContent($content, string $status = 'ok', int $code = 200) {
        $content = [
          'status' => $status,
          'body' => $content,
          'code' => $code
    ...
    public function redirect($url) {
        if (empty($url)) {
            throw new \Exception('URL для перенаправления не передан');
        }
